...
consensus_messages: public({
    ...
    # Total deposit of this epoch, derived from subtracting collective penalties from deposit of last epoch
    total_deposit: wei_value,
    # Deposit of every validator in this epoch
    validator_deposit: wei_value[num]
}[num]) # index: epoch
...
def initiate():
    ...
    # Set total deposit of epoch 0
    self.consensus_messages[0].total_deposit = as_wei_value(3, ether)
    # Set deposit of initial validator
    self.consensus_messages[0].validator_deposit[0] = as_wei_value(3, ether)
    # Set initial total deposit counter
    # self.total_deposits[0] = as_wei_value(3, ether)
    self.total_deposits[0] = self.consensus_messages[0].total_deposit
    # Set deposit scale factor
    self.consensus_messages[0].deposit_scale_factor = 1000000000000000000.0
    # Set penalty factor of epoch 0
    self.penalty_factor[0] = 0.01
...
def initialize_epoch(epoch: num):
    ...
    # Set penalty factor and subtract penalty from this epoch
    # Penalty factor of this epoch is based on performance(how many validators offline) of last epoch
    self.penalty_factor[epoch] = self.penalty_factor[self.current_epoch - 1]
    self.consensus_messages[epoch].total_deposit = floor(self.consensus_messages[epoch - 1].total_deposit * (1 - self.penalty_factor[epoch]))
    ...
    if self.consensus_messages[epoch - 1].committed:
        ...
        # update last dynasty deposit and move on to next dynasty
        self.total_deposits[self.dynasty] = self.consensus_messages[epoch-1].total_deposit
        self.dynasty += 1
        # self.total_deposits[self.dynasty] = self.total_deposits[self.dynasty - 1] + self.next_dynasty_wei_delta
        # subtract penalty from current deposit
        self.consensus_messages[epoch].total_deposit = floor((self.consensus_messages[epoch - 1].total_deposit + self.next_dynasty_wei_delta) * (1 - self.penalty_factor[epoch]))
        self.total_deposits[self.dynasty] = self.consensus_messages[epoch].total_deposit
    ...
    #subtract penalty from logged-out deposit
    self.next_dyan_to_subtract = floor(self.next_dyan_to_subtract * (1 - self.penalty_factor[epoch]))
    self.seconde_next_dyn_to_subtract = floor(self.seconde_next_dyn_to_subtract * (1 - self.penalty_factor[epoch]))
...
def flick_status(logout_msg: bytes <= 1024):
    ...
    if login_flag:
        ...
        self.validators[validator_index].latest_updated_epoch = 0
    # Logging out
    else:
...
def subtract_validator_deposit(validator_index: num):
    if self.validators[validator_index].dynasty_start > self.dynasty:
        return
    ...
    if self.validators[validator_index].latest_updated_epoch == 0 and self.dynasty_start_epoch[self.validators[validator_index].dynasty_start] > 0:
    ...
    # specify until which epoch should the validator's deposit to be updated
    ...
    elif self.validators[validator_index].dynasty_end == self.dynasty:
        end_epoch = self.dynasty_start_epoch[self.dynasty] - 1
    else:
        end_epoch = self.current_epoch
    total_epochs = end_epoch - self.validators[validator_index].latest_updated_epoch
    ...
    # After update validator's deposit should be consistent with the deposit recorded in latest updated epoch
    assert self.validators[validator_index].deposit == self.consensus_messages[end_epoch].validator_deposit[validator_index]
...
def prepare(prepare_msg: bytes <= 1024):
    ...
    if not (self.current_epoch > epoch or (in_prev_dynasty and not in_current_dynasty)):
        self.subtract_validator_deposit(validator_index)
    # epoch of previous dynasty
    epoch_prev_dyn = self.dynasty_start_epoch[self.dynasty_in_epoch[epoch]]-1
    ...
    if self.current_epoch == epoch:  #if blockhash(epoch * self.epoch_length) == hash:
        # Logic here:
        # reward goes to deposit of current epoch if validator is valid in current dynasty
        # if validator is only valid in previous dynasty, then reward goes to epoch of previous dynasty
        if (in_prev_dynasty and not in_current_dynasty):
            reward = floor(self.validators[validator_index].deposit * self.reward_factor * 2/3 * 1/4)
            self.validators[validator_index].deposit += reward
            self.consensus_messages[epoch_prev_dyn].total_deposit += reward
            self.consensus_messages[epoch_prev_dyn].validator_deposit[validator_index] += reward
            self.total_deposits[self.dynasty_in_epoch[epoch_prev_dyn]] += reward
        else:
            reward = floor(self.validators[validator_index].deposit * self.reward_factor)
            self.validators[validator_index].deposit += reward
            self.consensus_messages[epoch].total_deposit += reward
            self.consensus_messages[epoch].validator_deposit[validator_index] += reward
        # Need to sync up with the deposit that's going to be subtracted
        # because validator can still prepare/commit when validator logged out
    ...
    if in_current_dynasty:
        curdyn_prepares += self.consensus_messages[epoch].validator_deposit[validator_index]
    ...
    if in_prev_dynasty:
        prevdyn_prepares += self.consensus_messages[epoch_prev_dyn].validator_deposit[validator_index]
    ...
    if (curdyn_prepares >= self.consensus_messages[epoch].total_deposit * 2 / 3 and \
    ...
    raw_log([self.prepare_log_topic], concat(as_bytes32(self.consensus_messages[epoch].prev_dyn_prepares[sourcing_hash]), as_bytes32(self.total_deposits[self.dynasty_in_epoch[epoch]-1])))
    raw_log([self.prepare_log_topic], concat(as_bytes32(self.consensus_messages[epoch].prepares[sourcing_hash]), as_bytes32(self.consensus_messages[epoch].total_deposit)))
...
def commit(commit_msg: bytes <= 1024):
    ...
    # Update validator deposit
    if not (in_prev_dynasty and not in_current_dynasty):
        self.subtract_validator_deposit(validator_index)
    ...
    if True:  #if blockhash(epoch * self.epoch_length) == hash:
        ...
        if (in_prev_dynasty and not in_current_dynasty):
            epoch_prev_dyn = self.dynasty_start_epoch[self.dynasty_in_epoch[epoch]]-1
            reward = floor(self.validators[validator_index].deposit * self.reward_factor * 2/3 * 3/4)
            self.validators[validator_index].deposit += reward
            self.consensus_messages[epoch_prev_dyn].total_deposit += reward
            self.consensus_messages[epoch_prev_dyn].validator_deposit[validator_index] += reward
            self.total_deposits[self.dynasty_in_epoch[epoch_prev_dyn]] += reward
    ...
    if in_prev_dynasty:
        ...
        self.consensus_messages[epoch].prev_dyn_commits[ancestry_hash] += self.consensus_messages[self.dynasty_start_epoch[self.dynasty_in_epoch[epoch]]-1].validator_deposit[validator_index]
    ...
    # if (self.consensus_messages[epoch].commits[ancestry_hash] >= self.total_deposits[self.dynasty] * 2 / 3 and \
    if (self.consensus_messages[epoch].commits[ancestry_hash] >= self.consensus_messages[epoch].total_deposit * 2 / 3 and \
            self.consensus_messages[epoch].prev_dyn_commits[ancestry_hash] >= self.total_deposits[self.dynasty - 1] * 2 / 3) and \
    ...
    raw_log([self.commit_log_topic], concat(as_bytes32(self.consensus_messages[epoch].commits[ancestry_hash]), as_bytes32(self.consensus_messages[epoch].total_deposit)))
