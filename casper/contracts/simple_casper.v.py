...
@constant
def get_deposit_size_in(validator_index: num, epoch: num) -> num(wei):
    start_epoch = self.dynasty_start_epoch[self.validators[validator_index].dynasty_start]
    current_deposit = self.validators[validator_index].deposit * 1.0
    # counting deposit size backward, starting from current epoch
    for i in range(self.current_epoch - epoch + 1):
        if bitwise_and(self.commit_bitmap[self.current_epoch - i][validator_index / 256],
                       shift(as_num256(1), validator_index % 256)):
            current_deposit = current_deposit / (1 + self.current_penalty_factor[self.current_epoch - i] * \
                                                     self.consensus_messages[self.current_epoch - i].deposit_scale_factor / \
                                                     self.consensus_messages[start_epoch].deposit_scale_factor)
        if bitwise_and(self.prepare_bitmap[self.current_epoch - i][validator_index / 256],
                       shift(as_num256(1), validator_index % 256)):
            current_deposit = current_deposit / (1 + self.current_penalty_factor[self.current_epoch - i] * \
                                                     self.consensus_messages[self.current_epoch - i].deposit_scale_factor / \
                                                     self.consensus_messages[start_epoch].deposit_scale_factor)
...
def prepare(prepare_msg: bytes <= 1024):
    ...
    if (self.current_epoch == epoch and self.ancestry_hashes[epoch] == ancestry_hash) and \
        ...
        # Record that validator has prepared in this epoch in time
        self.prepare_bitmap[epoch][validator_index / 256] = \
            bitwise_or(self.prepare_bitmap[epoch][validator_index / 256],
                       shift(as_num256(1), validator_index % 256))
